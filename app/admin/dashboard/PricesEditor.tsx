 ...
 *   • « Unique »            → un seul prix SANS libellé (montant + volume) ;
 ...
 * prêt pour `savePrices`. (Inspiré de app/admin/carte/PricesEditor.tsx, non lié.)
 */
...
export default function PricesEditor({
  ...
  // Remonte le tableau reconstruit à chaque évolution de l'état interne.
  useEffect(() => {
    onChange(build(mode, amount, vol, rows));
    // onChange est stable (mémo côté parent) ; on ne suit que l'état local.
  ...
  return (
    <div className="rounded-xl border-2 border-bb-white/25 p-3">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <span className="font-body text-[0.62rem] uppercase tracking-[0.28em] text-bb-white/75">
          Tarif
        </span>
      ...
      {mode === "single" ? (
        ...
        <div className="flex flex-col gap-2">
          {rows.map((r, i) => (
            <div key={i} className="flex items-end gap-2">
              ...
              <button
                ...
                className="mb-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border-2 border-bb-white/25 text-bb-white/80 transition-colors hover:border-bb-red hover:text-bb-white"
              >
                ✕
              </button>
            </div>
          ...
          <Button
            variant="ghost"
            onClick={() => setRows((prev) => [...prev, { label: "", amount: "" }])}
